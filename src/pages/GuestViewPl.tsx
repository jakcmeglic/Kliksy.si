  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (isUploadingRef.current) {
        e.preventDefault();
        e.returnValue = 'Zdjęcia są nadal przesyłane. Czy na pewno chcesz zamknąć aplikację?';
    ...
    const uploadTask = async () => {
      try {
        ...
        if (successCount > 0) {
          ...
          setUploadError("Nie udało się przesłać żadnego zdjęcia. Spróbuj ponownie.");
          setUploadProgress({ current: 0, total: 0 });
        }
      } catch (error: any) {
        setUploadProgress({ current: 0, total: 0 });
        const errorMessage = error.message ? `Błąd: ${error.message}` : "Wystąpił błąd podczas przesyłania. Spróbuj ponownie.";
  ...
  if (!event) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 text-center">
        <h2 className="text-3xl font-bold mb-4">Wydarzenie nie istnieje</h2>
        <p className="text-gray-600 mb-8">Sprawdź link lub kod QR.</p>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <header className="px-6 py-8 text-center bg-white rounded-b-3xl shadow-sm border-b border-gray-100 relative z-10">
        ...
        <p className="text-gray-500 text-sm">
          {event.welcomeMessage || (event.eventType === 'poroka' || !event.eventType ? 'Dziękujemy za dzielenie się z nami wspomnieniami.' : 'Dziękujemy za dzielenie się z nami wspomnieniami.')}
        </p>
      </header>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                ...
                <span className="text-xl font-bold">Zrób zdjęcie</span>
              </button>
              ...
              >
                ...
                <span className="text-xl font-bold">Prześlij z galerii</span>
              </button>
          ...
          >
            <div className="flex items-center justify-between mb-4 px-2">
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-lg text-gray-900">{event?.guestViewSettings === 'own' ? 'Moje zdjęcia' : 'Ostatnie wspomnienia'}</h3>
                <span className="text-xs font-medium bg-indigo-50 px-2 py-1 rounded-full text-indigo-600">Na żywo</span>
              </div>
