...
export default function DashboardHr() {
  ...
  const handleDownloadAll = async () => {
    ...
    setDownloadError('');
    setDownloadProgress('Pripremam ZIP... X/Y slika'.replace('X', '0').replace('Y', photos.length.toString()));
    ...
    try {
      ...
      for (let i = 0; i < photos.length; i++) {
        ...
        try {
          ...
          successCount++;
          setDownloadProgress('Pripremam ZIP... X/Y slika'.replace('X', successCount.toString()).replace('Y', totalCount.toString()));
        } catch (err) {
          console.error("Failed to fetch image for ZIP:", photo.url, err);
        }
      }
      
      setDownloadProgress('Stvaranje datoteke...');
      ...
      setDownloadError('Došlo je do greške prilikom preuzimanja. Pokušajte ponovno.');
      setIsDownloading(false);
    }
  };
