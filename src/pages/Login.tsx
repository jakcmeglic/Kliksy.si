  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'forgot_password') {
        ...
        setSuccessMsg("Povezava za ponastavitev gesla je bila poslana na vaš elektronski naslov.");
        setAuthMode('login');
      } else if (authMode === 'register') {
        const { signUpWithEmail } = await import('../firebase');
        await signUpWithEmail(email, password, 'sl');
        ...
        >
          <div className="text-center mb-8">
            <h1 className="text-3xl font-serif text-gray-900 mb-2">
              {authMode === 'login' ? 'Dobrodošli nazaj' : authMode === 'forgot_password' ? 'Ponastavitev gesla' : 'Ustvarite račun'}
            </h1>
            <p className="text-gray-500">
              {authMode === 'login' ? 'Prijavite se za dostop do vašega dogodka' : authMode === 'forgot_password' ? 'Vnesite e-poštni naslov in poslali vam bomo povezavo za ponastavitev gesla' : 'Pridružite se nam in ustvarite nepozabne spomine'}
            </p>
          </div>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (authMode === 'login' ? 'Prijava' : authMode === 'forgot_password' ? 'Pošlji povezavo' : 'Registracija')}
            </button>
          </form>
