...
export default function Dashboard() {
  ...
  const handleDownloadVideos = async () => {
    ...
    setIsDownloading(true);
    setDownloadError('');
    setDownloadProgress('Preverjam velikosti videov...');
    ...
    try {
      ...
      alert(`Videov za prenos: ${videoFiles.length}. Prenos bo razdeljen v ${batches.length} ZIP datotekah (500-700MB vsaka).`);
      ...
      for (let b = 0; b < batches.length; b++) {
        ...
        for (let i = 0; i < batch.length; i++) {
          ...
          setDownloadProgress(`Pripravljam ZIP videov ${b + 1}/${batches.length}: ${i + 1}/${batch.length} videov...`);
        }
        ...
        <div className="p-6 flex-1">
          ...
          <div className="flex flex-col gap-2">
            <button onClick={() => navigate('/create')} className="w-full py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl font-medium text-sm flex items-center justify-center gap-2 transition">
              <Plus className="w-4 h-4" /> Nov dogodek
            </button>
        ...
        <nav className="mt-4 px-4 pb-4">
          <ul className="space-y-1.5">
            <li>
              ...
              >
                <LayoutGrid className="w-5 h-5" /> Nadzorna plošča
              </button>
            </li>
            <li>
              <button
