...
export default function DashboardHr() {
  ...
  const handleDownloadVideos = async () => {
    ...
    setIsDownloading(true);
    setDownloadError('');
    setDownloadProgress('Sprawdzanie rozmiarów wideo...');
    ...
    try {
      ...
      alert(`Wideo do pobrania: ${videoFiles.length}. Pobieranie zostanie podzielone na ${batches.length} plików ZIP (500-700MB każdy).`);
      ...
      for (let b = 0; b < batches.length; b++) {
        ...
        setDownloadProgress(`Przygotowywanie pliku ZIP z wideo ${b + 1}/${batches.length}...`);
        ...
        for (let i = 0; i < batch.length; i++) {
          ...
          setDownloadProgress(`Przygotowywanie pliku ZIP z wideo ${b + 1}/${batches.length}: ${i + 1}/${batch.length} wideo...`);
        }
      ...
      if (totalAdded === 0) {
        alert('Żadne wideo nie zostało dodane do ZIP. Sprawdź konsolę pod kątem błędów.');
      }
    } catch (err: any) {
      console.error('ZIP error:', err);
      alert('Błąd podczas pobierania: ' + err.message);
    } finally {
      setIsDownloading(false);
    }
  };
              ...
              <div className="mt-3 flex flex-col items-start gap-2">
                ...
                {event.paymentStatus === 'paid' && event.plan !== 'premium' && (
                  <button onClick={() => navigate('/upgrade')} className="mt-2 text-sm text-indigo-600 hover:text-indigo-700 font-medium flex items-center gap-1">
                    Ulepsz pakiet <ChevronRight className="w-4 h-4" />
                  </button>
                )}
              </div>
            )}
        ...
        <nav className="mt-4 px-4 pb-4">
          <ul className="space-y-1.5">
            <li>
              ...
              >
                <LayoutGrid className="w-5 h-5" /> Panel główny
              </button>
            </li>
            <li>
              <button
              ...
              >
                <ImageIcon className="w-5 h-5" /> Galeria ({photos.length})
              </button>
            </li>
            <li>
              <button
              ...
              >
                <QrCode className="w-5 h-5" /> Kod QR
              </button>
            </li>
            <li>
              <button
              ...
              <li>
                ...
                >
                  <span className="flex items-center gap-3">
                    <Sparkles className="w-5 h-5" /> Ulepsz 
                  </span>
            ...
            >
                 ...
                 <div className="flex flex-wrap items-center gap-3">
                   {photos.filter(f => !isVideo(f)).length > 0 && (
                     ...
                     >
                       ...
                       📸 Pobierz zdjęcia ({photos.filter(f => !isVideo(f)).length})
                     </button>
                   )}
                   {photos.filter(f => isVideo(f)).length > 0 && (
                     <button
                     ...
                     >
                       ...
                       🎥 Pobierz filmy ({photos.filter(f => isVideo(f)).length})
                     </button>
                   )}
                 </div>
               </div>
                   ...
                   >
                     ...
                     <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/0 to-black/0 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                        <div className="flex items-center justify-between">
                           <p className="text-white text-sm font-medium truncate max-w-[120px]">{photo.uploadedBy || 'Gość'}</p>
                           <button
                             onClick={(e) => {
                               e.stopPropagation();
                     ...
                     <button
                       onClick={(e) => {
                         e.stopPropagation();
                         if(window.confirm('Ali ste prepričani, da želite izbrisati to zdjęćo?')) {
                           deletePhoto(photo);
                         }
                       }}
               ...
               {photos.length === 0 && (
                 <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm">
                   ...
                   <h3 className="text-xl font-bold text-gray-900 mb-2">Brak zdjęć</h3>
                   <p className="text-gray-500">Brak zdjęć. Udostępnij kod QR gościom!</p>
                 </div>
               )}
            </motion.div>
          )}
          ...
          {activeTab === 'settings' && (
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-700">Kdo lahko vidi zdjęće v galeriji na telefonu gosta?</label>
                  <select 
                  ...
                  >
                    <option value="all">Vse zdjęće</option>
                    <option value="own">Samo svoje zdjęće</option>
                  </select>
                  <p className="mt-2 text-sm text-gray-500">
                    Če izberete "Samo svoje zdjęće", gostje v galeriji (ko skenirajo kodo) ne bodo videli zdjęć drugih gostov, ampak samo tiste, ki so jih sami naložili.
                  </p>
                </div>
