...
function restoreTabs(file, isPl, isHr) {
  ...
  const regex = /<div className="max-w-5xl mx-auto">[\s\S]*?<QRModal/;
  
  let qrmodalName = 'QRModal';
  if (isHr) qrmodalName = 'QRModalHr';
  if (isPl) qrmodalName = 'QRModalPl';

  const replaceRegex = new RegExp(`<div className="max-w-5xl mx-auto">[\\s\\S]*?<${qrmodalName}`);
  ...
  // Fix any duplicate div max-w-5xl issue
  content = content.replace(/<div className="max-w-5xl mx-auto">\s*<div className="max-w-5xl mx-auto">/g, '<div className="max-w-5xl mx-auto">');

  fs.writeFileSync(file, content, 'utf8');
}

restoreTabs('src/pages/Dashboard.tsx', false, false);
restoreTabs('src/pages/DashboardHr.tsx', false, true);
